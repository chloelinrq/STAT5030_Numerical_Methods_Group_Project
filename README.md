...
For the MBS valuation process, we now have two key inputs: short rate paths and CPR paths.Now, our future cash flows follow the equation:$$CF_t = \text{Interest}_t + \text{Scheduled Principal}_t + \text{Prepayment}_t$$Where:$\text{Interest}_t = \text{Balance}_{t-1} \times q$, where $q = \frac{\text{Coupon Rate}}{12}$$\text{Scheduled Principal}_t = \min(\text{PMT} - \text{Interest}_t, B_{t-1})$$\text{Prepayment}_t = \text{SMM}_t \times (B_{t-1} - \text{Scheduled Principal}_t)$We discount the cash flows back using the discount factors ($DF_t$) derived from the Hull-White short-rate paths:$$PV = \sum_{t=1}^{T} CF_t \times DF_t$$Finally, we compute the average Present Value across all $N$ simulated paths to get the price of MBS:$$\text{Value} = \frac{1}{N} \sum_{i=1}^{N} PV_i$$
---
